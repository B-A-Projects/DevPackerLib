...
public struct FilesystemTable: Codable {
    
    /// Represents the number by which file
    /// offsets need to be multiplied.
    var fileOffsetFactor: UInt32 { get { return _fileOffsetFactor } }
    
    /// Contains the filesystem table groups
    /// defined in this filesystem table
    ...
    /// Contains the key used for decrypting
    /// both the filesystem table and the content
    /// defined in it. This key may be undefined
    /// for unencrypted files, or files encrypted
    /// on partition level.
    ...
    internal var _getGroup: (_ groupEntry: FileSystemTableGroupHeader,
                            _ fileOffset: UInt64,
                            _ fileSize: UInt64
    ) throws -> [UInt8]
    
    public init(from decoder: any Decoder) throws {
        let values = try decoder.container(keyedBy: FilesystemKeys.self)
        _fileOffsetFactor = try values.decodeIfPresent(UInt32.self, forKey: .fileOffsetFactor) ?? 0x20
        _groupHeaders = try values.decodeIfPresent([FileSystemTableGroupHeader].self, forKey: .groupHeaders) ?? []
        _entryTable = try values.decodeIfPresent(FileSystemTableEntry.self, forKey: .entryTable)!
        _key = try values.decodeIfPresent([UInt8].self, forKey: .key)
        
        _getGroup = { groupEntry, fileOffset, fileSize in return Array(repeating: 0, count: 0) }
    }
    ...
    public init(File reader: Reader,
                ...
                GetGroup: @escaping (_ groupEntry: FileSystemTableGroupHeader,
                                     _ fileOffset: UInt64,
                                     _ fileSize: UInt64
                ) throws -> [UInt8],
    ...
    public func Extract(FilesystemTableIndex filesystemTableIndex: UInt32,
        ...
        return try tableEntry!.extract(DestinationUrl: destination,
    ...
    internal func getFile(ChunkIndex index: UInt32,
        ...
        let chunk = try _getGroup(groupEntry,
...
//        }
    ...
    public func encode(to encoder: any Encoder) throws {
        var container = encoder.container(keyedBy: FilesystemKeys.self)
        try container.encode(_fileOffsetFactor, forKey: .fileOffsetFactor)
        try container.encode(_groupHeaders, forKey: .groupHeaders)
        try container.encode(_entryTable, forKey: .entryTable)
